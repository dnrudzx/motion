...
NO3, NE3 = headkeypoints("3")
maxX3, maxY3 = Framescale('/home/ms/frame/frame_30.jpg')
x5, y5 = NoseKeypoint(NO3, maxX3, maxY3)
x6, y6 = NeckKeypoint(NE3, maxX3, maxY3)
...
def head_point_out(tooH, tooL, ok):
    ...
    for i in range(len(tooH)+len(tooL)+len(ok)):
        path = ('/home/ms/frame/frame_3'+ str(i) +'.jpg')
        ...
        if i in tooH:
            print("머리를 너무 높이 들었습니다. 머리를 내려주세요")
        elif i in tooL:
            print("머리를 너무 내렸습니다. 머리를 올려주세요")
        else:
            print("머리 높이가 정확합니다! ")
    cv2.destroyAllWindows()


a = getDegree(x, y, x2, y2)
b = getDegree(x5, y5, x6, y6)
c, d, e = sHead(a, b)

head_point_out(c,d,e)
...
def Framematch():
    ...
    abs(maxHC - minHC) abs(maxHC - minHC) 
Framematch()
'''
